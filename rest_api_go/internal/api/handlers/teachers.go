...
func GetTeachersHandler(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		if err != nil {
			http.Error(w, "Error scaning databse results", http.StatusInternalServerError)
			return
		}
...
func GetOneTeacherHandler(w http.ResponseWriter, r *http.Request) {
	...
	var teacher models.Teacher
	err = db.QueryRow("SELECT id, first_name, last_name, email, class, subject FROM teachers WHERE id = ?", id).Scan(&teacher.ID, &teacher.FirstName, &teacher.LastName,
...
func PatchTeachersHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, update := range updates {
		...
		_, err = tx.Exec("UPDATE teachers SET first_name = ?, last_name = ?, email = ?, class = ?, subject = ? WHERE id = ?", teacherFromDb.FirstName, teacherFromDb.LastName, teacherFromDb.Email,
			teacherFromDb.Class, teacherFromDb.Subject, teacherFromDb.ID)
		if err != nil {
			tx.Rollback()
			http.Error(w, "Error updating teacher", http.StatusInternalServerError)
			return
