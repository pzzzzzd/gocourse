...
func GetExecByID(id int) (models.Exec, error) {
	...
	var exec models.Exec
	err = db.QueryRow("SELECT id, first_name, last_name, email, username, inactive_status, role FROM execs WHERE id = ?", id).Scan(&exec.ID, &exec.FirstName, &exec.LastName,
		&exec.Email, &exec.Username, &exec.InactiveStatus, &exec.Role)
...
func PatchExecs(updates []map[string]interface{}) error {
	...
	for _, update := range updates {
		...
		_, err = tx.Exec("UPDATE execs SET first_name = ?, last_name = ?, email = ?, username = ? WHERE id = ?", execFromDb.FirstName, execFromDb.LastName, execFromDb.Email, execFromDb.ID,
			execFromDb.Username)
		if err != nil {
			tx.Rollback()
...
func PatchOneExec(id int, updates map[string]interface{}) (models.Exec, error) {
	...
	err = db.QueryRow("SELECT id, first_name, last_name, email, username FROM execs WHERE id = ?", id).Scan(&existingExec.ID, &existingExec.FirstName, &existingExec.LastName,
		&existingExec.Email, &existingExec.Username)
	if err != nil {
		if err == sql.ErrNoRows {
			log.Println(err)
	...
	_, err = db.Exec("UPDATE execs SET first_name = ?, last_name = ?, email = ?, username = ? WHERE id = ?", existingExec.FirstName, existingExec.LastName, existingExec.Email,
		existingExec.ID, existingExec.Username)
	if err == sql.ErrNoRows {
		log.Println(err)
